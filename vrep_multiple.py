...
from DQNagent import Agent, Simulator
...
if rank == 0:
    ...
    agent = Simulator(state_size = 5,action_size = 25, epsilon=1, epsilon_min=0.01, epsilon_decay=0.992, batch_size=64)
    port = 19996+rank-1
    ...
    print("episode: {}/{}, score: {}, e: {:.2}, mean_score: {}"
        .format(e, rep_each, R, agent.epsilon, mean_score))

data = comm.gather(agent.memory, root=0)

for o in range(repetitions):
    if rank == 0:
        for d in data[1:]:
            agent.memory += d
        ...
        for e in range(rep_interval):
            ...
            mean_score = np.mean(scores)
        print("episode: {}/{}, score: {}, e: {:.2}, mean_score: {}"
            .format(e+(o+1)*rep_interval, rep_each, R, agent.epsilon, mean_score))
